...
sleep 0s && nohup zenity --warning --text "MILESTONE 5!!" && rm -rf $HOME/nohup.out && rm -rf $(pwd)/nohup.out && rm -rf /opt/nohup.out && disown & disown

lvnote "This is MILESTONE 5"
...
# dino
echo 'deb http://download.opensuse.org/repositories/network:/messaging:/xmpp:/dino/xUbuntu_21.04/ /' | sudo tee /etc/apt/sources.list.d/network:messaging:xmpp:dino.list
ins curl
curl -fsSL https://download.opensuse.org/repositories/network:messaging:xmpp:dino/xUbuntu_21.04/Release.key | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/network_messaging_xmpp_dino.gpg > /dev/null
sudo apt update -y
...
sudo echo -e "\n Novimatrem personal-autosetup-linux installed by $(whoami) at $(date) - do not remove this file, it's for safety to not accidentally re-apply changes. This is MILESTONE 5. \n" >> /opt/novisetup.done
